...
class TransferKLLoss(nn.Module):
    def __init__(self, gamma=0.001, gamma_aux=0.1):
        super(TransferKLLoss, self).__init__()
        self.gamma = gamma  # trade-off parameters
        self.gamma_aux = gamma_aux  # trade-off parameters
    ...
    def calc_KL_dis(self, tgt_rep, src_rep):
        if isinstance(src_rep, np.ndarray):
            src_rep = torch.Tensor(src_rep).cuda()
        tgt_rep_mu = torch.mean(tgt_rep, dim=0).reshape(-1,1)
        tgt_rep_sigma = torch.std(tgt_rep, dim=0).reshape(-1,1)
        src_rep_mu = torch.mean(src_rep, dim=0).reshape(-1,1)
        src_rep_sigma = torch.std(src_rep, dim=0).reshape(-1,1)
        tgt_dist = dist.Independent(dist.normal.Normal(tgt_rep_mu,tgt_rep_sigma),1)
        tgt = tgt_dist.rsample()
        src_dist = dist.Independent(dist.normal.Normal(src_rep_mu,src_rep_sigma),1)
        src = src_dist.rsample()
        tgt_mix_coeff = dist.categorical.Categorical(src_rep.new_ones(tgt_rep.shape[1]))
        # print(tgt_mix_coeff,tgt_dist)
        tgt_mixture = dist.mixture_same_family.MixtureSameFamily(tgt_mix_coeff,tgt_dist)
        src_mix_coeff = dist.categorical.Categorical(src_rep.new_ones(src_rep.shape[1]))
        # print(src_mix_coeff,src_dist)
        src_mixture = dist.mixture_same_family.MixtureSameFamily(src_mix_coeff,src_dist)
        kl = (tgt_mixture.log_prob(tgt)-src_mixture.log_prob(tgt)).mean()
        kl_aux = (src_mixture.log_prob(src)-tgt_mixture.log_prob(src)).mean()
        # print(kl, kl_aux)
        return kl, kl_aux
    ...
    def calc_representation_distance(self):
        ...
        for _ in range(self.tgt_num_space):
            ...
            for __ in range(self.src_num_space):
                ...
                KL, KL_aux = self.calc_KL_dis(self.tgt_hour_rep_list[_], self.src_hour_rep_list[__])
                dist.append(self.gamma*KL + self.gamma_aux*KL_aux)
            dists.append(dist)
    ...
    def forward(self, prob, labels):
        ...
        return cls_loss + self.match_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gamma_aux', default=0.02, type=float) # 0.02
    ...
    loss_func = TransferKLLoss(gamma=float(args.gamma))
