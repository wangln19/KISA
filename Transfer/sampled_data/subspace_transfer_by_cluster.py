...
def generate_tgt_representation(model, data_loader, num_clusters = 5):
    ...
    model_0 = KMeans(n_clusters=num_clusters, random_state=0).fit(reps[indices_0].cpu())
    model_1 = KMeans(n_clusters=num_clusters, random_state=0).fit(reps[indices_1].cpu())
    ...
    month_list = np.unique(cluster_indicator)
    month_centroid_list = []
    for mon in month_list:
        indices = np.where(cluster_indicator==mon)[0]
        month_centroid_list.append(reps[indices].mean(axis=0))         
    return month_list, month_centroid_list           

def load_source_domain_representation(source_rep_name, num_clusters=5):
    '''
    ...
    model_0 = KMeans(n_clusters=num_clusters, random_state=0).fit(data["rep"][indices_0])
    model_1 = KMeans(n_clusters=num_clusters, random_state=0).fit(data["rep"][indices_1])
    model_1.labels_  = model_1.labels_ + num_clusters
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mark', default="subcluster", type=str)
    ...
    model_name = args.datasets + "_" + testpath.split("/")[-1].split("_")[-1].replace(".csv","") + "_" + args.mark + "_subcluster" + args.para + ".pt" 
    da_model_name = args.datasets + "_" + testpath.split("/")[-1].split("_")[-1].replace(".csv","") + "_" + args.mark + "_da.pt"
    ...
    src_month_list, src_month_centroid_list, src_labels = load_source_domain_representation(source_model_name, num_clusters=num_clusters)
    tgt_month_list, tgt_month_centroid_list = generate_tgt_representation(model, train_dataset, num_clusters = num_clusters)
