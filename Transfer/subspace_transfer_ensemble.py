...
class TransferLoss(nn.Module):
    def __init__(self, src_month_list, src_month_centroid_list, tgt_month_list, gamma=0.001):
...
def eval(model, eval_loader, optimizer, epoch, tgt_month_list, tgt_month_centroid_list, loss_func=nn.CrossEntropyLoss, desc='Validation', verbose=True, model_name='best_model.pt', save_model_num=5):
    ...
    heapq_object = checkpoint["heapq"]
    model_item = (auc, {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch, "best_auc": auc,"best_spauc":spauc})
    if len(heapq_object) < save_model_num:
        print("pushing item in heap without poping ... pushing item auc is:{}".format(auc))
        heapq.heappush(heapq_object, model_item)
    else:
        if auc > min_best_auc:
            min_best_auc = auc
            pop_item = heapq.heappushpop(heapq_object, model_item)
            print("poping item in heap... pop item auc is:{}".format(pop_item[0]))
            print("pushing item in heap... push item auc is:{}".format(auc))
        else:
            # update epoch
            print("updating epoch...")
    state = {'heapq': heapq_object, 'epoch': epoch}
    torch.save(state, model_name)


def eval_with_ensemble(model, loader, epoch, desc='Validation', verbose=True, model_name='best_model.pt'):
    validation_accuracy = 0
    validation_epoch_size = 0
    label_list = []
    prob_list = []
    ...
    checkpoint = torch.load(model_name)
    heapq_object = checkpoint["heapq"]
    print("there are {} model in the heapq.".format(len(heapq_object)))
    for i in range(len(heapq_object)):
        print("evaluating {} model...".format(i+1))
        model_item = heapq_object[i][1]
        model.load_state_dict(model_item["model"]) 
        ...
        auc = roc_auc_score(label_list, prob_list)
        spauc = roc_auc_score(label_list, prob_list,max_fpr=0.01)
        print(f'Epoch {model_item["epoch"]}, Validation AUC: {auc}, Validation SPAUC: {spauc}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mark', default="ensemble", type=str)
    ...
    if os.path.exists(model_name):
        checkpoint = torch.load(model_name)  # {"epoch":epoch,"heapq":heapq_object}
        heapq_object = checkpoint["heapq"]
        
        best_model_in_heap = heapq.nlargest(1,heapq_object)[0][1] # return the best model to continue training
 
        model.load_state_dict(best_model_in_heap['model'])
        optimizer.load_state_dict(best_model_in_heap['optimizer'])
        start = checkpoint["epoch"] + 1
        max_best_auc = best_model_in_heap["best_auc"]
        min_best_auc = heapq.nsmallest(1, heapq_object)[0][0] # return the min best auc
        print("start epoch:",start)
        print("max_best_auc:",max_best_auc)
        print("min_best_auc:",min_best_auc)
    else:
        start = 0
        min_best_auc = 0
        # init state
    ...
    loss_func = TransferLoss(src_month_list, src_month_centroid_list, tgt_month_list, gamma= float(args.gamma))
    ...
    if args.mode == 'train':
        ...
        eval_with_ensemble(model, eval_loader, epoch, verbose=True, model_name=model_name)
        print("evaluating test set...")
        eval_with_ensemble(model, test_loader, epoch, verbose=True, model_name=model_name)

    elif args.mode == 'validate':
        print("evaluating val set...")
        eval_with_ensemble(model, eval_loader, start, verbose=True, model_name=model_name)
        print("evaluating test set...")
        eval_with_ensemble(model, test_loader, start, verbose=True, model_name=model_name)