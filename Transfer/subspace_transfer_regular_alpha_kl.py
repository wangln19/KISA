...
class TransferLoss(nn.Module):
    def __init__(self, src_month_list, src_month_centroid_list, gamma=0.001):
    ...
    def update_tgt_representation(self, tgt_month_list, tgt_month_centroid_list, model):
        ...
        mean_weight = (torch.ones((self.src_num_spaces, self.tgt_num_space)) * (1/self.src_num_spaces)).cuda(device=device)
        alpha_regularization_loss = F.kl_div(alpha.log(), mean_weight, reduction='sum')
        self.alpha_regularization_loss = alpha_regularization_loss
        # alpha_regularization_loss = alpha.std(dim=0, unbiased =False)
        # self.alpha_regularization_loss = alpha_regularization_loss.sum()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mark', default="regular_kl", type=str)
