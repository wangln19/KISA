...
def generate_tgt_representation(model, data_loader, target_month_name, amount_split_bins=10):
    ...
    amount_indicator = np.arange(amount_split_bins) + 1
    amount_centroid_list = []
    for ind in range(amount_split_bins):
...
def load_source_domain_representation(source_rep_name,amount_split_bins=10):
    '''
    ...
    amount_indices_list = []
    print("amount_split_bins:",amount_split_bins)
    num_bin = num_records // amount_split_bins
    for i in range(amount_split_bins):
        amount_indices_list.append(sorted_index[i*num_bin:(i+1)*num_bin])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--amount_split_bins', default=10,type=int)
    # parser.add_argument('--filepath', default='./data/HK.pkl')
   
    parser.add_argument('--mark', default="amount", type=str)
    ...
    src_month_list, src_month_centroid_list, src_labels = load_source_domain_representation(source_model_name,amount_split_bins=args.amount_split_bins)
    tgt_month_list, tgt_month_centroid_list = generate_tgt_representation(model, train_dataset, target_month_name,amount_split_bins=args.amount_split_bins)
