...
def generate_tgt_representation(model, data_loader, target_month_name, amount_split_list=[500,5000,50000]):
    ...
    with open(target_month_name + "_amount.pkl","rb") as fp:
        amount_list = pickle.load(fp)

    amount_indices_list = []
    num_space = 1 + len(amount_split_list)
    # less than first element
    amount_indices_list.append(np.where(amount_list<amount_split_list[0])[0])

    for i in range(len(amount_split_list)-1):
        amount_indices_list.append(np.where((amount_list>amount_split_list[i])&(amount_list<=amount_split_list[i+1]))[0])
    
    # larger than last element
    amount_indices_list.append(np.where(amount_list>amount_split_list[-1])[0])

    amount_indicator = np.arange(num_space) + 1
    amount_centroid_list = []
    for ind in range(num_space):
        indices = amount_indices_list[ind]
        amount_centroid_list.append(reps[indices].mean(axis=0))
    
    return amount_indicator, amount_centroid_list          


def load_source_domain_representation(source_rep_name,amount_split_list=[500,5000,50000]):
    '''
    ...
    with open(source_rep_name + "_rep.pkl","rb") as fp:
        ...
        reps = data["rep"]
    with open(source_rep_name.replace("lstm","amount.pkl"),"rb") as fp:
        amount_list = pickle.load(fp)
    

    amount_indices_list = []
    print("amount_split_list:", amount_split_list)
    num_space = 1 + len(amount_split_list)
    # less than first element
    amount_indices_list.append(np.where(amount_list<amount_split_list[0])[0])
    ...
    return amount_indicator, amount_centroid_list, labels           
...
if __name__ == '__main__':
    ...
    parser.add_argument('--amount_split_list', default="[500,5000,50000]",type=str)
    # parser.add_argument('--filepath', default='./data/HK.pkl')
   
    parser.add_argument('--mark', default="amount_list", type=str)
    args = parser.parse_args()
    args.amount_split_list = [int(x) for x in args.amount_split_list.replace("[","").replace("]","").split(",")]
    ...
    src_month_list, src_month_centroid_list, src_labels = load_source_domain_representation(source_model_name,amount_split_list=args.amount_split_list)
    tgt_month_list, tgt_month_centroid_list = generate_tgt_representation(model, train_dataset, target_month_name,amount_split_list=args.amount_split_list)
