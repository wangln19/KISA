...
def generate_tgt_representation(model, data_loader, num_clusters=5):
    ...
    model = KMeans(n_clusters=num_clusters, random_state=0).fit(reps.cpu())

    month_indicator = model.labels_
...
def load_source_domain_representation(source_rep_name, num_clusters = 5):
    ...
    model = KMeans(n_clusters=num_clusters, random_state=0).fit(data["rep"])
    month_indicator = model.labels_
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mark', default="subclusterwolabel", type=str)
