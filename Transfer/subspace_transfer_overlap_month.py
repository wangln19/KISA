...
def generate_tgt_representation(model, data_loader, target_month_name):
    ...
    with open(target_month_name + "_day.pkl","rb") as fp:
        day_indicator = pickle.load(fp)
        day_15_indices = calcu_day_15_indices(day_indicator)

    num_month = len(np.unique(month_indicator))
    month_list = np.arange(2*num_month-1) + 1
    month_centroid_list = []
    # loading centroid of month - for day 1 to day 31
    for mon in range(num_month):
        indices = np.where(month_indicator==(mon+1))[0]
        month_centroid_list.append(reps[indices].mean(axis=0))
    
    for ind in range(num_month-1):
        month_centroid_list.append(reps[day_15_indices[ind]:day_15_indices[ind+1]].mean(axis=0))
...
def load_source_domain_representation(source_rep_name):
    ...
    with open(source_rep_name.replace("lstm","day.pkl"),"rb") as fp:
        day_indicator = pickle.load(fp)
        day_15_indices = calcu_day_15_indices(day_indicator)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mark', default="overlap_month", type=str)
